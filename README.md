...
本仓库已集成自用版“星星分析助手 v2.1”：在原 daily_stock_analysis 的单股 AI 分析能力上，增加 A 股全市场基本面优先筛选、证据质量层、估值/预期差层、交易机会层、现实模拟盘、持仓复盘和历史回测验证。

当前前端默认首页是策略池工作台，原单股分析页保留在 `/analysis`。系统定位是“AI 自动研究 + 规则化评分 + 模拟验证”，不是自动实盘交易。

详细运行逻辑、打分公式、默认阈值、定时任务、Docker 部署和常用命令见：[星星分析助手 v2.1 使用说明](docs/STAR_ASSISTANT_V21.md)。
